...
if (location.search.indexOf('targetId') > -1) {
    location.search.replace(/&?targetId=([^&]*)/, function(allMatch, targetIdGroup, index, rawStr) {
        targetId = targetIdGroup;
        return '';
    });
}
$(function() {
     $('#sidebar').resizable({
         ...
         stop: function(e, ui) {
             ...
             $('#code-container').css('width', sidebar.position()
                 .left);
         }
     });
    ...
    $('#eval').keydown(function(e) {
        ...
        switch (e.keyCode) {
            ...
            case 38: // Up
                if (!evalBig) {
                    e.preventDefault();
                    if (currentHistoryPos < historyArr.length) {
                        $('#eval').val(historyArr[
                            currentHistoryPos++]);
...
function toggleRedirectConsole() {
    postToServer({
        command: 'update-redirect-console',
        data: $('#redirectConsole').attr('checked') !== undefined
    });
}
...
function updateBreakpoints() {
    ...
    if (window.localStorage) {
        window.localStorage.setItem('breakpoints', JSON.stringify(
            breakpoints));
    }
    ...
    $(breakpoints).each(function(i, breakpoint) {
        ul.append(
            ...
            //.append('&nbsp;&nbsp;&nbsp;&nbsp;')
            .append(
                $('<span>').append(getLine(files[breakpoint[0]]
                    .file, breakpoint[1]))
            )
        );
    })
...
function addToFileSwitcher(filePath, fileLabel) {
    $('<option></option>').val(filePath).text(fileLabel).appendTo($(
        '#file-switcher'));
}
...
function postToServer(payload) {
    ...
    req.open('POST', '/desktop/outgoing?targetId=' + targetId, false);
...
function listenToServer() {
    var req = new XMLHttpRequest();
    req.open('GET', '/desktop/incoming?targetId=' + targetId, true);
...
function showFile(data) {
    ...
    tokenize(files[data.file].file || '', function(token, type) {
        ...
        if (type === 'word' && keywordList.indexOf(token) > -1) {
            ...
            post = '</span>';
        } else if (type === 'word' && literalList.indexOf(token) >
            -1) {
            pre = '<span class="literal">';
            post = '</span>';
        } else if (['string', 'comment', 'number'].indexOf(type) >
            -1) {
            pre = '<span class="' + type + '">';
            post = '</span>';
        }
    ...
    var codeLines = codeTokens
        ...
        .map(function(x, i) {
            ...
            var paddedNum = '<span class="linenum' + extraClass +
                '" file="' + data.file + '" line="' + num + '">' +
    ...
    if (textAreaContentHeight > textAreaHeight) {
        var scrollAmountPerLine = (textAreaContentHeight - textAreaHeight) /
            numLines;
...
function highlightLine(line, numLines) {
    ...
    $code.css({
        ...
        'background-size': '9px 7px, 100% ' + Math.round(
            heightPerLine) + 'px',
        'background-position': '5px ' + Math.round((line - 1) *
                heightPerLine + ((heightPerLine - 7) / 2)) + 'px, ' +
...
function toggleBreakpoint() {
    ...
    if (existsBreakpoint(file, line)) {
        ...
        breakpoints = breakpoints.filter(function(b) {
            return !(b[0] == file && b[1] == line);
        });
    } else {
...
function processOutput(data) {
    switch (data.command) {
        ...
        case 'print-eval-result':
            try {
                writeToConsole(
                    '<br/>&nbsp;&nbsp;&nbsp; <b>INPUT:</b> ' + data.input
                    .replace(/\b__this\b/, 'this') +
                    '<br/>&nbsp;&nbsp;&nbsp; <b>RESULT:</b> ' +
                    JSON.stringify(JSON.parse(data.result), null, '\t')
                    .replace(/\n/g, '<br>&nbsp;&nbsp;&nbsp;')
                    .replace(/\t/g, '&nbsp;&nbsp;'));
            } catch (e) {
                writeToConsole('<b>INPUT:</b> ' + data.input.replace(
                        /\b__this\b/, 'this') + ' <b>RESULT:</b> ' +
                    data.result);
            }
            break;
        case 'report-exception':
            writeToConsole('<b>EXCEPTION</b>: ' + data.message + ' at ' +
...
function writeToConsole(msg) {
    $('<div></div>').html((++lineNum) + ': ' + msg).appendTo($(
        '#output-inner'))[0].scrollIntoView();
}
